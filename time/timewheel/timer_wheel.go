...
func New(timeMs time.Duration, slot, bufferSize int) gotime.Timer {
	var w = &wheelTimer{
		timeMS:     timeMs,
...
func (w *wheelTimer) addTask(t *internal.Task) {
	// 对于周期性任务，可能存在这种场景：
...
func (w *wheelTimer) After(delay time.Duration, fn func()) (uint64, error) {
	if fn == nil {
...
func (w *wheelTimer) Every(delay time.Duration, fn func()) (uint64, error) {
	if fn == nil {
		return 0, gotime.ErrNilFunc
	}
	if w.isClosed() {
...
func (w *wheelTimer) Delete(id uint64) error {
	if id == 0 {
		return nil
	}
	if w.isClosed() {
...
func (w *wheelTimer) Cron(desc string, fn func()) (uint64, error) {
	if fn == nil {
		return 0, gotime.ErrNilFunc
	}
	if w.isClosed() {
